# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Описание проекта

Это проект для автоматизированного тестирования браузера с использованием стелс-технологий на примере Steam. Проект включает в себя модули для обхода детекции автоматизации, генерации реалистичных browser fingerprints, эмуляции человеческого поведения и работы с прокси.

⚠️ **ВАЖНО**: Это исследовательский проект для тестирования технологий обхода детекции. НЕ используйте для нарушения условий использования сервисов.

## Архитектура

### Основные компоненты

1. **steam_test_stealth.py** - главный скрипт для запуска тестового браузера со стелс-функциями:
   - Класс `SteamTestStealth` управляет сессией браузера
   - Функция `refresh_proxy_ip()` обновляет IP мобильных прокси через API
   - Функция `detect_proxy_geo()` определяет геолокацию прокси
   - Адаптеры для человекоподобного ввода и движений мыши

2. **src/stealth/** - модули для обхода детекции:
   - `fingerprint_generator.py` - генерация уникальных browser fingerprints (WebGL, hardware, viewport)
   - `cookie_generator.py` - создание реалистичных cookies с историей посещений
   - `storage_generator.py` - генерация localStorage данных для имитации реального браузера
   - `human_typing.py` - эмуляция человеческого набора текста с опечатками
   - `human_mouse.py` - эмуляция человеческих движений мыши по кривым Безье
   - `geo_config.py` - конфигурации геолокации для разных стран

3. **src/captcha/** - модули для решения капчи:
   - `yescaptcha_solver.py` - клиент для YesCaptcha API, автоматическое решение hCaptcha

4. **firefox_antidetect_extension/** - расширение Firefox для скрытия navigator.webdriver:
   - `manifest.json` - манифест расширения
   - `content_script.js` - скрипт, инжектируемый в страницы для скрытия признаков автоматизации

5. **steam_registration.py** - скрипт для автоматической регистрации аккаунтов Steam:
   - Заполнение формы регистрации с эмуляцией человеческого поведения
   - Автоматическое решение hCaptcha через YesCaptcha
   - Интеграция со всеми стелс-модулями

6. **steam_registration_batch.py** - пакетная регистрация множества аккаунтов:
   - Последовательная обработка аккаунтов из конфигурационного файла
   - Отслеживание состояния регистрации (можно продолжить после прерывания)
   - Автоматический retry при ошибках (до 3 попыток на аккаунт)
   - Детальная статистика по успешным и проваленным регистрациям

7. **src/utils/account_queue.py** - модуль управления очередью аккаунтов:
   - Загрузка и парсинг конфигурационного файла с аккаунтами
   - Сохранение/восстановление состояния обработки в JSON
   - Отслеживание статусов: pending, in_progress, completed, failed, captcha_failed, email_exists
   - Статистика и отчёты по обработке

## Команды разработки

```bash
# Запуск тестового браузера (откроется Firefox со стелс-функциями)
python steam_test_stealth.py

# Запуск без прокси
python steam_test_stealth.py --no-proxy

# Запуск в headless режиме (без GUI)
python steam_test_stealth.py --headless

# Регистрация одного аккаунта
python steam_registration.py

# Пакетная регистрация множества аккаунтов
python steam_registration_batch.py

# Пакетная регистрация с ручным решением капчи
python steam_registration_batch.py --manual-captcha

# Пакетная регистрация в headless режиме
python steam_registration_batch.py --headless

# Просмотр статистики обработки аккаунтов
python steam_registration_batch.py --stats

# Сброс провалившихся аккаунтов для повторной попытки
python steam_registration_batch.py --reset-failed
```

## Конфигурационные файлы

- **proxies.txt** - список прокси (по одному на строку в формате login:pass@host:port или host:port:login:pass)
- **proxy_config.txt** - URL для обновления IP мобильных прокси (API endpoint)
- **yescaptcha_config.txt** - API ключ для сервиса YesCaptcha (для автоматического решения капчи)
- **accounts.txt** - список аккаунтов для пакетной регистрации (формат: email:username:password)
- **accounts_state.json** - автоматически создаваемый файл состояния обработки аккаунтов
- **completed_accounts.txt** - автоматически создаваемый файл с успешно зарегистрированными аккаунтами

## Зависимости

Основные зависимости проекта:
- selenium - автоматизация браузера
- requests - HTTP запросы для работы с API прокси и геолокации

## Ключевые особенности реализации

### Обход детекции WebDriver

Проект использует двухуровневый подход:
1. Firefox extension инжектирует код на уровне document_start (до загрузки страницы)
2. JavaScript injection через Selenium после загрузки для дополнительных модификаций

### Генерация Fingerprint

`FingerprintGenerator` создаёт согласованные профили с реалистичными комбинациями:
- WebGL vendor/renderer специфичные для браузера (Chrome использует ANGLE, Firefox - нативные драйверы)
- Hardware конфигурации (cores, memory) соответствуют современным системам
- Viewport использует только популярные разрешения для уменьшения уникальности

### Работа с прокси

Поддерживаются HTTP и SOCKS5 прокси с автоматическим определением формата. При использовании мобильных прокси происходит:
1. Обновление IP через API перед запуском браузера
2. Определение геолокации нового IP
3. Настройка locale, timezone и currency в соответствии с геолокацией

### Человекоподобное поведение

- **HumanTypist**: эмулирует реалистичную скорость печати с вариативными задержками и опечатками
- **HumanMouse**: движения по кривым Безье с случайными отклонениями и ускорениями

### Автоматическое решение капчи

Проект поддерживает **несколько сервисов** для автоматического решения hCaptcha:

#### AZcaptcha (⚠️ НЕ РАБОТАЕТ с hCaptcha)
- ❌ **ВНИМАНИЕ**: AZcaptcha НЕ поддерживает hCaptcha (несмотря на документацию)
- Модуль `src/captcha/azcaptcha_solver.py` оставлен для совместимости
- Тестирование показало: `ERROR_INVALID_SITEKEY` на всех hCaptcha
- **НЕ рекомендуется** для production использования
- Подробности: `AZCAPTCHA_VERDICT.md`

#### YesCaptcha (backup)
- Метод `solve_captcha()` в `steam_registration.py:337` использует YesCaptcha API
- Класс `YesCaptchaSolver` в `src/captcha/yescaptcha_solver.py` поддерживает два режима:
  1. **Официальный SDK** (рекомендуется): `pip install yescaptcha` - автоматически используется если установлен
  2. **Fallback API**: прямое взаимодействие через requests, если SDK не установлен
- API ключ загружается из файла `yescaptcha_config.txt` или переменной окружения `YESCAPTCHA_API_KEY`
- Токен решения автоматически инжектируется в форму через JavaScript
- Подробная документация в файле `YESCAPTCHA_SETUP.md`
- Официальная документация: https://yescaptcha.atlassian.net/wiki/spaces/YESCAPTCHA/pages/30113813/HCaptcha+Python+selenium+DEMO

#### Universal Captcha Solver (рекомендуется)
- Модуль `src/captcha/universal_captcha_solver.py` поддерживает несколько сервисов:
  - **2Captcha** ⭐ (рекомендуется для production) - $2-3/1000, 10-20 сек
  - **CapSolver** ⭐ (для сложных капч) - $2.5-4/1000, 10-15 сек
  - **AntiCaptcha** (альтернатива) - $2-3/1000, 15-25 сек
  - YesCaptcha (fallback) - $1.5-2/1000, 20-30 сек
  - ~~AZcaptcha~~ (не работает с hCaptcha)
- Все сервисы протестированы и работают со Steam hCaptcha
- Тестовый скрипт: `test_2captcha_steam.py`

## Тестирование стелс-функций

Браузер открывается на странице Steam и остаётся активным для ручной проверки:
1. Откройте консоль разработчика (F12) и проверьте `navigator.webdriver` (должен быть undefined)
2. Проверьте fingerprint на pixelscan.net или других детекторах
3. Проверьте работу прокси и геолокацию

## Структура данных прокси

Поддерживаемые форматы в proxies.txt:
- `login:pass@host:port` - стандартный формат
- `host:port:login:pass` - альтернативный формат
- `socks5://login:pass@host:port` - SOCKS5 прокси
- `http://login:pass@host:port` - HTTP прокси с явным указанием протокола